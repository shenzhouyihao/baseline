...
# -*- coding: utf-8 -*-
"""
RAFT 长期预测实验驱动脚本 - 8 数据集完整版
数据集: ETTh1/ETTh2/ETTm1/ETTm2 + Weather/Electricity/Exchange/Traffic
窗口:   seq_len=96, pred_len in {96, 192, 336, 720}
输出:   raft_results_8datasets.csv  (model,dataset,seq_len,pred_len,mse,mae)

模型源自 https://github.com/archon159/RAFT.git
已修改 exp/exp_long_term_forecasting.py 在训练前自动调用 prepare_dataset()
...
启动: setsid nohup /home/cxq/anaconda_envs/py3.9-torch2.5.1/bin/python -u \
        run_raft_8datasets.py > driver_raft_8datasets.log 2>&1 < /dev/null &
"""
import csv
import glob
import os
...
def main():
    ...
    for name, root_path, data_path, enc_in, ds_args in DATASETS:
        for pred_len in PRED_LENS:
            ...
            # ETT 数据集需要指定 data='ETTh1'/'ETTh2' 等
            data_arg = name if name.startswith('ETT') else 'custom'
