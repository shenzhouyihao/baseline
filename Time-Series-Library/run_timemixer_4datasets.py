...
# -*- coding: utf-8 -*-
"""
TimeMixer 长期预测实验驱动脚本
数据集: Weather / Electricity / Exchange / Traffic
窗口:   seq_len=96, pred_len in {96, 192, 336, 720}
输出:   timemixer_results.csv  (model,dataset,seq_len,pred_len,mse,mae)

超参数沿用 scripts/long_term_forecast/*_script/TimeMixer.sh 官方配置。
Exchange 缺官方脚本，按该目录下其他模型的通例组合参数。
...
启动: setsid nohup /home/cxq/anaconda_envs/py3.9-torch2.5.1/bin/python -u \
        run_timemixer_4datasets.py > driver_timemixer.log 2>&1 < /dev/null &
"""
import csv
import glob
import os
...
MODEL = 'TimeMixer'
CSV_PATH = os.path.join(ROOT, 'timemixer_results.csv')
LOG_DIR = os.path.join(ROOT, 'logs_timemixer')
...
# (CSV数据集名, root_path, data_path, 变量数, 该数据集共用参数)
DATASETS = [
    ('Weather',     './dataset/weather/',       'weather.csv',        21,
     ['--e_layers', '3', '--d_model', '16', '--d_ff', '32', '--batch_size', '128',
    ...
    ('Electricity', './dataset/electricity/',   'electricity.csv',   321,
     ['--e_layers', '3', '--d_model', '16', '--d_ff', '32', '--batch_size', '32',
      '--learning_rate', '0.01', '--train_epochs', '20', '--patience', '10',
      '--down_sampling_layers', '3', '--down_sampling_window', '2', '--down_sampling_method', 'avg']),
    ('Exchange',    './dataset/exchange_rate/', 'exchange_rate.csv',   8,
     ['--e_layers', '2', '--d_model', '16', '--d_ff', '32', '--batch_size', '32',
      '--learning_rate', '0.01', '--train_epochs', '10', '--patience', '5',
      '--down_sampling_layers', '3', '--down_sampling_window', '2', '--down_sampling_method', 'avg']),
    ('Traffic',     './dataset/traffic/',       'traffic.csv',       862,
     ['--e_layers', '3', '--d_model', '32', '--d_ff', '64', '--batch_size', '8',
      '--learning_rate', '0.01', '--train_epochs', '10', '--patience', '5',
      '--down_sampling_layers', '3', '--down_sampling_window', '2', '--down_sampling_method', 'avg']),
]
